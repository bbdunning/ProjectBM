...
void DynamicCharacterController::setup (btScalar height, btScalar width, btScalar stepHeight)
{
	...
	startTransform.setOrigin (btVector3(0.0, 3.0, -5.0));
...
void DynamicCharacterController::preStep (const btCollisionWorld* collisionWorld)
{
	...
	xform = m_rigidBody->getWorldTransform();
	btVector3 down = -xform.getBasis()[1];
	// btVector3 forward = xform.getBasis()[2];
	btVector3 forward = forwardDir;
	down.normalize ();
	forward.normalize();
...
btVector3 DynamicCharacterController::getLocation() {
	btTransform xform;
	m_rigidBody->getMotionState()->getWorldTransform (xform);
...
void DynamicCharacterController::setForwardDir(const btVector3 &fd)
{
	forwardDir = fd;
}